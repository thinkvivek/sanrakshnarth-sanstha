...
def fetch_constraints(cursor, table_name):
    ...
    """
    query = f"""
    SELECT CONSTRAINT_NAME, CONSTRAINT_TYPE, COLUMN_NAME, R_CONSTRAINT_NAME
...
def create_temp_table(cursor, original_table, temp_table, columns):
    ...
    for column in columns:
        ...
        elif column[1].upper() == 'NUMBER':
            if column[3] is not None and column[4] is not None:
                column_def += f"({column[3]},{column[4]})"
            elif column[3] is not None:
                column_def += f"({column[3]})"
...
def main():
    ...
    # Connect to the source and target databases using context managers
    with connect_to_db(ppp_username, ppp_password, ppp_dsn) as ppp_conn, \
         connect_to_db(vbd_username, vbd_password, vbd_dsn) as vbd_conn:
